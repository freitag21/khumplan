...
## 2026-08-28 — รีวิวกฎหมาย kaimook: consent hardening + Terms/Privacy/DPA (commit c80e1e3, migration 0010)

ไข่มุก (subagent ทนาย) ตรวจ 5 เรื่องต้องปิดก่อนเก็บเงิน + คปภ. (ประเมินว่าอยู่ฝั่งปลอดภัย) + BOT (PromptPay ไม่ต้องขอใบอนุญาต) **แก้ในโค้ด/ข้อความแล้ว:**
- **migration 0010**: `policy_acceptances` (append-only · RLS read/insert own ไม่มี update/delete = แก้/ลบไม่ได้) · `clients.sensitive_consent`/`_at` (ม.26 แยกจากทั่วไป) · `analyses.consent_version`/`consent_at` · trigger `freeze_client_consent` (ล็อกร่องรอย consent ลูกค้า)
- signIn/signUp → `recordPolicyAcceptance` เขียนแถวเมื่อเวอร์ชันที่ยอมรับล้าสมัย · `saveAnalysis` ประทับ consent_version/at
- `renderNewClient`: consent แยก 2 ช่อง — (บังคับ) ทั่วไป + (เลือก) ยินยอมโดยชัดแจ้งข้อมูลสุขภาพ ม.26 · หน้าลูกค้ามีแบนเนอร์ + ปุ่ม "บันทึกความยินยอมข้อมูลสุขภาพ" เมื่อยังไม่ได้
- **dashboard: ปุ่มเปิด/ปิดลิงก์แชร์รายผลวิเคราะห์** (`setShareEnabled`) — ทำให้สิทธิระงับการใช้ ม.34 ใช้งานได้จริง
- `POLICY_VERSION` → `2569-08-28.2`
- **หน้า `?view=dpa` ใหม่** (`renderDpa`) — DPA เต็ม: ขอบเขตคำสั่ง (ห้ามใช้เพื่อวัตถุประสงค์ตัวเอง), sub-processor รวม Resend, แจ้งเหตุละเมิด 24 ชม., ช่วย DSR 7 วัน, ฐานโอนต่างประเทศ, audit, exit 30/90 วัน
- **Terms** (ร่าง): ผู้มีสิทธิใช้ = มีใบอนุญาตเท่านั้น (ผู้อบรม = ข้อมูลสมมติ) · ข้อ compete แคบลง · "แหล่งรายได้ของเรา" (ไม่รับเงินจากบริษัทประกัน) · อ้าง DPA + 24 ชม. · Team section ขยาย (คู่สัญญา, ยินยอมข้อมูลกิจกรรม, "ไม่มอบสมุดลูกค้าให้หัวหน้าทีมแม้ร้องขอ", แจ้ง 15 วันถ้าไม่จ่าย) · หัวข้อค่าบริการ · cap ความรับผิด (ค่าบริการ 12 ด. / ฟรี ฿5k) · แจ้งเปลี่ยน 30 วัน + คืนเงินตามสัดส่วน · เขตอำนาจศาลไทย · IP = ภาษาผู้ควบคุมข้อมูล
- **Privacy** (ร่าง): แยกฐาน ม.19 / ม.26 · ฐานโอนต่างประเทศ ม.28 · เพิ่ม Resend · แจ้งเหตุ 24 ชม. · ข้อมูลผู้รับประโยชน์/ผู้เยาว์ · ผู้ประสานงาน PDPA
- landing "ปลอดภัยเรื่อง คปภ." → "ออกแบบโดยคำนึงถึงแนวปฏิบัติ" · หน้าสนับสนุน "ฟรีต่อไป" → ตรงกับ landing · disclaimer หน้าผล + "ไม่ใช่เอกสารเสนอขาย/ไม่ใช่คำแนะนำการลงทุน"

migration 0010 รันแล้ว (verified). push + deploy แล้ว
**อีเมลโดเมน: `support@khumplan.com` LIVE** (commit d268ff8) — Cloudflare Email Routing (Claude ตั้งผ่าน Chrome) · MX + SPF + DKIM เพิ่มแล้ว · forward → Gmail · `CONTACT.email` เปลี่ยนแล้ว · Resend outbound ไม่กระทบ · **เหตุผลเดิม "apex MX เสี่ยง" เป็นความเข้าใจผิด** — MX คนละชนิดกับ A record

**งานของ founder (ไข่มุกระบุว่าต้องทำเอง — ไม่ใช่งานโค้ด):** จ้างทนาย practising ตรวจชุด Terms+Privacy+DPA รอบเดียวก่อนออกใบแจ้งหนี้ใบแรก (งบ 15,000–40,000 บาท) · จดทะเบียนพาณิชย์อิเล็กทรอนิกส์ (50 บาท ครึ่งวัน) + แยกบัญชีรับเงินธุรกิจ · ใส่ชื่อ-นามสกุลจริง + ที่อยู่ใน `OWNER_LABEL` (ตอนนี้เป็น "ผู้พัฒนา คุ้มแพลน") · **อ่านสัญญาตัวแทนของตัวเองเรื่องข้อห้ามแข่งขัน/ขัดผลประโยชน์ ก่อนขายดีลแรก** (ไข่มุกชี้ว่าผลกระทบร้ายแรงสุด) · เมื่อ ARR ~300k → จดบริษัท + หนังสือโอน IP เข้าบริษัท · ยื่นเครื่องหมายการค้า (จำพวก 42, ยื่นเป็นเครื่องหมายผสม, ค้นก่อน) · ตัดสินใจเรื่องคำว่า "ยูนิตลิงก์" บนหน้าขาย (ตอนนี้ใส่ disclaimer ไว้แล้ว) · ดาวน์โหลด DPA/SCC ของ Supabase+Vercel เก็บ · ทำ ROPA 1–2 หน้า
**ยังไม่ทำ (polish):** cron auto-delete retention · re-consent modal บังคับ (ตอนนี้บันทึกเงียบ ๆ ตอน sign-in) · PIN หน้าแชร์ · archive snapshot ข้อความ Terms แต่ละเวอร์ชัน (git history ใช้แทนได้)
